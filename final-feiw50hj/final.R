...
name <- "Jiwook Yoo"
github.name <- "jiwooky"
email.address <- "jiwooky@berkeley.edu"
...
set.seed(344)
#m <- <your code here>
m = matrix(rnorm(100, mean = 8), ncol = 10, nrow = 10)
...
#cw4 <- <your code here>
cw4 = ChickWeight[sample(1:length(ChickWeight$Time), 200), ]  
...
co = lm(mtcars$qsec ~ mtcars$hp)$coefficients
plot(mtcars$qsec ~ mtcars$hp)
abline(a = co[[1]], b = co[[2]])
# [3 pt]
...
plot(mtcars$wt, mtcars$disp, col = c("blue", "red"), xlab = "weight of the car in lb/1000",
     ylab = "engine displacement in cu. in")
...
hist(WorldBank$GDP[WorldBank$lending == "IDA"], breaks = floor(max(WorldBank$GDP[WorldBank$lending == "IDA"], na.rm = TRUE)/250), 
     xlab = "GDP", main = "GDP among IDA rated countries")
...
max.diff = sapply(rain, diff)
abs.max = sapply(max.diff, abs)
max.diff.rain = sapply(abs.max, max)
...
q = prop(c(1:4, 0 , 4 , 2, NA))
prop.rain = sapply(rain, prop)
...
par(mfrow=c(2,3))
rainplot = function(x){
  hist(x, xlab = "rainfall in inches", main=("Histogram of rainfall"))
}
sapply(rain, rainplot)
#################################################################
...
##   <your code here>  
## }
standardizeVar = function(m, cols){
  if(missing(cols))
    cols = TRUE
  if(cols){
    means = c(1:ncol(m))
...
## }
PermDiff = function(cases, controls, k){
  if(missing(k))
    k = 5000
  sample()
}
...
NumJackpot = function(k, B){
  ...
  n.jackpot = c(1:5000)
  for(c in 1:B){
    winners = sort(sample(c(1:19), size = 3, replace = FALSE))
    for(e in 1:k){
    player = sort(sample(c(1:19), size = 3, replace = FALSE))
    if(isTRUE(all.equal(winners, player)))
       count = count + 1
        round = round + 1
      }
    n.jackpot[c] = round
...
text1 = phrases[grep("^(st)", phrases, ignore.case = TRUE)]
...
text3 <- phrases[grep("((ta){1,})", phrases)]
# [2 pts]
...
abbreviate <- function(vector, k){
  ab.vector = substring(vector ,1, k)
  return(ab.vector)
}
text4 = abbreviate(phrases, 3)
# [2 pts]

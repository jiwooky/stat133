...
# Output : words, a character vector where each element is one word 
  ...
  sentences = gsub(pattern = "[0-9]+s*", replacement = "", sentences)
  sentences = tolower(sentences)
  # Drop the words (Applause. and Laughter.)
  sentences=gsub(pattern = "\\[applause\\]", replacement = "", sentences, ignore.case = TRUE)
  sentences=gsub(pattern = "\\[laughter\\]", replacement = "", sentences, ignore.case = TRUE)
...
wordVecs <-lapply(speechWords, toVector)
...
tempchar = sapply(speechWords, nchar)
chars <- sapply(tempchar, sum)
...
collect = function(a){
  if(length(wordMat[ ,presidents == a]) == no.uniqueWords)
    return(wordMat[ ,presidents == a])
  return(rowSums(wordMat[ ,presidents == a]))
  }
presidentWordMat <- sapply(unique(presidents), collect)
...
presParty=c()
pres = colnames(presidentWordMat)
for(c in 1:length(pres)){
  pres[c] = as.character(speechesDF[speechesDF$Pres == pres[c], "party"][1])
}
...
plot(mds, xlab = "", ylab = "")
text(mds, labels = pres ,color = cols)
...
plot(speechesDF$year, speechesDF$sentences, xlab="Year", ylab="sentences")
plot(speechesDF$year, speechesDF$words, xlab="Year", ylab="words")
plot(speechesDF$year, speechesDF$chars, xlab="Year", ylab="characters")
plot(speechesDF$year, speechesDF$chars/speechesDF$words, xlab="Year", ylab="mean word length")
plot(speechesDF$year, speechesDF$words/speechesDF$sentences,xlab="Year", ylab="mean sentence length")

...
dice_sum <- function(k=2, B=100){
  dsum = replicate(B, sum(sample(c(1:6), k, replace = TRUE)))
  return(dsum)
}
...
hotelCal.split <- tolower(unlist(strsplit(gsub("[[:punct:]]", "", hotelCal), " +")))
...
updateDate <- function(dates, old.yr){
    oldyear = grep(old.yr, dates)
    updated.dates = gsub(old.yr, '2015', dates[oldyear])
